...
public function showForgotPasswordTeacher()
{
    return view('teacher_forgot_password');
}
protected function sendResetLinkEmailTeacher(Request $request)

{
    // Validate the email format and check if it exists in the teachers table
    $validator = Validator::make($request->all(), [
        'email' => [
            'required',
            'email',
            Rule::exists('teachers', 'email'), // Specify the column name for exists rule
        ],
    ]);
    ...
    Mail::send('emails.TeacherforgetPassword', ['token' => $token], function ($message) use ($request) {
...
public function showResetPasswordFormTeacher($token) { 
    return view('teacher_reset_password', ['token' => $token]);
 }



public function submitResetPasswordFormTeacher(Request $request)
{
    $request->validate([
    ...
    $resetPasswordEntry = DB::table('teacher_password_resets')
    ...
    DB::table('teacher_password_resets')
        ->where('email', $request->email)
        ->where('token', $request->token)
        ->update(['used_at' => now()]);

    // Redirect to the login page with a success message
    return redirect('/teacherLogin')->with('success', 'Your password has been changed. You can now login');
} 
